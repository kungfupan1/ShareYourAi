...
## 十四 【架构铁律：全局反硬编码规范】(AI 开发者必读)

> **对 AI 编程助手的最高指令**：在根据本文档生成、重构或完善任何代码时，你必须严格遵守“配置与代码绝对分离（12-Factor App）”的原则。如果你在代码中写死了任何属于以下分类的数据，将被视为**严重的代码事故**。

### 1. 严禁硬编码的“三大高压线”
* **网络与环境拓扑**：绝对禁止在代码中直接写入 `localhost`、`127.0.0.1`、端口号（如 `8000`、`6379`）、数据库连接串、Redis URL 或任何具体的内外网域名（如 `https://shareyouai...`）。
* **业务“魔法数字”（Magic Numbers）**：绝对禁止在业务逻辑中直接写死业务参数。包括但不限于：费率比例、具体金额（如扣费 `0.07`、提现门槛 `10` 元）、分页大小、超时时间（如 `300s`）、重试次数（如 `3` 次）。
* **外部依赖与凭证**：绝对禁止在代码中写死第三方 API Key、Secret、Token、特定的前端 DOM 选择器（如 `button[aria-label="Upload"]`）或固定的回调地址。

### 2. 强制要求的替代方案
当你需要使用上述数据时，必须采取以下标准做法：
* **系统级配置（第一优先级）**：对于网络、环境、密钥等，必须通过 Python 的 `os.getenv()` 或前端的环境变量机制读取，并在输出代码时，同步提供 `.env.example` 或 `docker-compose.yml` 的环境变量声明补充。
* **动态业务配置（第二优先级）**：对于业务规则、魔法数字、DOM 选择器等，必须设计通过数据库配置表（如 `PluginSystemConfig`）动态读取。
* **全局常量池（兜底方案）**：如果暂时不需要存入数据库，必须在模块的顶部（或专门的 `config.py` / `constants.js` 文件中）抽取为全大写的全局常量，并附带详细注释，绝对不能散落在函数内部。

**🚨 交付验收口令**：
在你输出完任何一段代码后，请主动进行一次自检，并在回复的末尾加上这句声明：
*“✅ 我已执行反硬编码自检。本段代码涉及的环境变量已提取，业务常量已归拢，不存在隐藏的魔法数字。”*